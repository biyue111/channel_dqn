...
# plt.ion()
iteration = channelConfig.iteration
            ...
            if AGENT_ACT_POLICY == 'Determinate':
                ...
                for i in range(len(index) - 1, 0, -1):
                    ...
                    if finded_index >= 2:
                        break
                """ Action decision """
                relative_score = [pred[max_indeices[x]] - pred[index[0]] for x in range(2)]
                channel_p = [relative_score[x] / (sum(relative_score)) for x in range(2)]
                # ran = random.random()
                # if ran >= p_aggre[0] and ran < p_aggre[1]:
                #     return max_indeices[0]
                # else:
                #     return max_indeices[1]
                a = numpy.random.choice(max_indeices, 1, p=channel_p)
                return a[0]
    ...
    def run(self, agent):
        ...
        for i in range(0, iteration):
            ...
            for k in range(USER_CNT):
                ...
                print("stuff to be observed#############",
                      (np.array(state_batch_ls[k]), ls_a[k], r, np.array(next_state_batch_ls[k])))
            ...
            agent.replay()
            for k in range(USER_CNT):
                record_str = str(k) + ',' + str(ls_s[k]) + ',' + str(ls_a[k]) + ',' + str(r) + ',' + str(
                    ls_s_[k]) + '\n'
